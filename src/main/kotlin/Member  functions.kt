...
Sometimes you need to store in objects not only the data itself but also a behavior.
Member functions implement a common behavior for the whole set of objects which belong to the same class.

Member functions look like functions placed in the class body.
In the example below, we declare a class with one function print():

class MyClass {
    fun print() = println("Hello from print")
}

This function is called a member function because it works with a specific object of a class and can access its fields.
It's a class member:
...
As you may remember, the keyword this represents the current instance of the class.
In the example above, it's an optional keyword, so you can omit it.
...
Note, member function is Kotlin's official name of functions that are placed inside a class. Such functions are often called methods.

Calling member functions
Let's take a look at the example below.
It prints the value of the field of the particular object:

val myObject = MyClassWithProperty(10)
myObject.printProperty()  // prints "10"
So to call a member function, you need to create an object of the class.
Then, put a dot after the object name and write the function name with the arguments that you need in parentheses.

An example with cats
As a more complex example, let's consider a class that represents a cat.
...
zzz
meow
So, member functions allow programmers to manipulate particular objects of a class and perform some operations.
As always, functions unite a sequence of operations under a name that makes sense.
 */
